...
<article class="min-w-0 rounded-lg border border-teal-100 bg-teal-50/60 p-4">
    <div class="flex flex-wrap items-start justify-between gap-3">
        <div class="min-w-0">
            <p class="truncate text-sm font-semibold text-slate-950">{{ $documentTitle }}</p>
            <p class="mt-1 text-xs font-medium text-teal-700">{{ $pageLabel }}</p>
        </div>
        @if ($showScore)
            <span class="rounded-full bg-white px-2.5 py-1 text-xs font-semibold text-slate-600 ring-1 ring-teal-100">
