    ...
    <header class="border-b border-slate-200 bg-white/90 backdrop-blur">
        <div class="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
            ...
            <nav class="hidden items-center gap-7 text-sm font-medium text-slate-600 md:flex">
                ...
                <a href="{{ route('documents.index') }}" class="hover:text-indigo-700">Documents</a>
                <a href="{{ route('chat.index') }}" class="hover:text-indigo-700">Chat</a>
            </nav>

            <div class="flex items-center gap-3">
                @auth
                    <a href="{{ route('documents.create') }}" class="rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow-sm shadow-indigo-200 hover:bg-indigo-700">
                        Upload PDF
                    </a>
                @else
                    <a href="{{ route('login') }}" class="hidden text-sm font-semibold text-slate-700 hover:text-indigo-700 sm:inline">Log in</a>
                    <a href="{{ route('register') }}" class="rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow-sm shadow-indigo-200 hover:bg-indigo-700">
                        Register
                    </a>
                @endauth
            </div>
