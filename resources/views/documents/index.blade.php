...
@section('content')
    ...
    <form method="GET" action="{{ route('documents.index') }}" class="mt-6 rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
        <div class="grid gap-3 md:grid-cols-[1fr_220px_140px]">
            <input name="search" type="search" value="{{ $search }}" placeholder="Search by title or filename" class="rounded-lg border border-slate-300 px-3 py-2.5 text-sm outline-none focus:border-teal-500 focus:ring-4 focus:ring-teal-100">
