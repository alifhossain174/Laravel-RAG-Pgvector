...
>
    <form method="POST" action="{{ route('password.store') }}" class="space-y-5">
        ...
        <label class="block">
            ...
            <input id="email" type="email" name="email" value="{{ old('email', $request->email) }}" required autofocus autocomplete="username" class="mt-2 w-full rounded-lg border border-slate-300 bg-white px-3 py-2.5 text-sm outline-none focus:border-teal-500 focus:ring-4 focus:ring-teal-100">
            @error('email')
